...
class ShellGPTClient:
    ...
    def completion(self, prompt: str, model: Optional[str] = None, temperature: float = 0.0,
    ...
    def load_model(self, model: str) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(f"{self.base_url}/api/v1/models/load", json=data)
            return self._handle_response(response)
        except requests.exceptions.RequestException as e:
            raise ShellGPTConnectionError(f"Failed to connect to server: {e}")
    ...
    def get_current_model(self) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/api/v1/model/current")
            return self._handle_response(response)
        except requests.exceptions.RequestException as e:
            raise ShellGPTConnectionError(f"Failed to connect to server: {e}")
...
if __name__ == "__main__":
    # Initialize client
    client = ShellGPTClient(api_key="your-api-key")
